...
Describe 'Remove-RegistrySubKeyTree Integration Tests using TestRegistry' -Tag 'Integration' {

    AfterEach {
        Remove-Item -Path TestRegistry:\ -Recurse -ErrorAction SilentlyContinue
    }

    It 'Should remove the subkey Tree successfully when given valid inputs' {
        ...
        $regPath = $registryPath[1]

        # Act
        Remove-RegistrySubKeyTree -RegistryHive $registryHive -RegistryPath $RegPath -Confirm:$false

        # Assert
        Test-Path "TestRegistry:\TestLocation\TestSubKey" | Should -BeFalse

        Test-Path "TestRegistry:\TestLocation" | Should -BeFalse
    }
    ...
    It 'Should throw an error if the subkey does not exist' {
        ...
        # Act & Assert
        { Remove-RegistrySubKeyTree -RegistryHive $registryHive -RegistryPath "Software\Pester\$invalidSubKey" -Confirm:$false } | Should -Throw
    }
    ...
    It 'Should support the ByKey parameter set' {
        ...
        $registryPath = ($path -split "HKEY_CURRENT_USER\\")
        $testRegistryPath = (New-Item -Path "TestRegistry:\TestLocation" -Name TestSubKey).pspath
        $testRegistryPath2 = (New-Item -Path "TestRegistry:\TestLocation\TestSubkey" -Name TestSubKey2).pspath
        ...
        # Act
        Remove-RegistrySubKeyTree -ParentKey $parentKey -SubKeyName $subKeyName -Confirm:$false

        # Assert
        Test-Path "TestRegistry:\TestLocation\TestSubKey\TestSubKey2" | Should -BeFalse
        Test-Path "TestRegistry:\TestLocation\TestSubKey" | Should -BeFalse
    ...
    It 'Should not remove subkey if ShouldProcess returns false' {
        ...
        Mock Invoke-DeleteSubKeyTree -ParameterFilter {
            $PSCmdlet.ShouldProcess($registryPath[1], 'Removing registry subkey') -eq $false
        } -ModuleName $script:dscModuleName

        # Act
        Remove-RegistrySubKeyTree -RegistryHive $registryHive -RegistryPath $registryPath[1] -Confirm:$false -WhatIf

        # Assert
        Test-Path "TestRegistry:\TestLocation\TestSubKey" | Should -BeTrue

        Assert-MockCalled -CommandName Invoke-DeleteSubKeyTree -Exactly 0 -Scope It -ModuleName $script:dscModuleName
